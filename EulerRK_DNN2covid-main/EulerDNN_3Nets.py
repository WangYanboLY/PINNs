...
class EulerDNN(tn.Module):
    ...
    def evalue_EulerDNN_FixedParas(self, t=None, s_init=10.0, i_init=10.0, r_init=10.0, d_init=10.0, size2predict=7,
        ...
        if opt2fixed_paras == 'last2train':
            ...
            nn_beta = scale2beta*torch.mean(nn2beta[:, 0], dim=0)
            nn_gamma = scale2gamma*torch.mean(nn2gamma[:, 0], dim=0)
            nn_mu = scale2mu*torch.mean(nn2mu[:, 0], dim=0)
...
def solve_SIRD(R):
    ...
    # params2Net = itertools.chain(SIRDmodel.BetaNN.parameters(), SIRDmodel.GammaNN.parameters(), SIRDmodel.MuNN.parameters())
...
if __name__ == "__main__":
    ...
    if R['model2NN'] == 'Fourier_DNN':
        R['hidden_layers'] = (75, 60, 50, 50, 30)  # 1*125+250*200+200*200+200*100+100*100+100*50+50*1=128205
    else:
        R['hidden_layers'] = (150, 60, 50, 50, 30)  # 1*250+250*200+200*200+200*100+100*100+100*50+50*1=128330
