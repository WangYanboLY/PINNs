...
class EulerDNN(tn.Module):
    def __init__(self, input_dim=4, out_dim=1, hidden_layer=None, Model_name='DNN', name2actIn='relu',
                 ...
                 factor2freq=None, sFourier=1.0, repeat_highFreq=False, use_gpu=False, No2GPU=0):
        ...
        if 'DNN' == str.upper(Model_name):
            ...
            self.DNN = DNN_base.Dense_ScaleNet(
                ...
                repeat_Highfreq=repeat_highFreq, type2float=type2numeric, to_gpu=use_gpu, gpu_no=No2GPU)
    ...
    def Quasi_Euler(self, t=None, input_size=100, s_obs=None, i_obs=None, r_obs=None, d_obs=None, loss_type='l2_loss',
                    scale2lncosh=0.5, scale2beta=1.0, scale2gamma=0.1, scale2mu=0.05):
        ...
        AI = torch.eye(input_size, dtype=self.float_type) * (-2)
        Ones_mat = torch.ones([input_size, input_size], dtype=self.float_type)
        A_diag = torch.linalg.band_part(Ones_mat, 0, 1)
        Amat = AI + A_diag

        dS2dt_obs = torch.matmul(Amat[0:-1, :], s_obs)
        dI2dt_obs = torch.matmul(Amat[0:-1, :], i_obs)
        dR2dt_obs = torch.matmul(Amat[0:-1, :], r_obs)
        dD2dt_obs = torch.matmul(Amat[0:-1, :], d_obs)

        nn_beta = scale2beta*torch.reshape(ParamsNN[:, 0], shape=[-1, 1])
        nn_gamma = scale2gamma*torch.reshape(ParamsNN[:, 1], shape=[-1, 1])
        nn_mu = scale2mu*torch.reshape(ParamsNN[:, 2], shape=[-1, 1])
        ...
        diff2S = dS2dt_obs - torch.reshape(nn_s2t, shape=[-1, 1])
        diff2I = dI2dt_obs - torch.reshape(nn_i2t, shape=[-1, 1])
        diff2R = dR2dt_obs - torch.reshape(nn_r2t, shape=[-1, 1])
        diff2D = dD2dt_obs - torch.reshape(nn_d2t, shape=[-1, 1])
    ...
    def Forward_Euler(self, t=None, input_size=100, s_obs=None, i_obs=None, r_obs=None, d_obs=None, loss_type='L2_loss',
                      scale2lncosh=0.5, scale2beta=1.0, scale2gamma=0.1, scale2mu=0.05):
        assert (t is not None)
        assert (s_obs is not None)
        assert (i_obs is not None)
        ...
        diff2S = torch.reshape(s_obs[1: input_size], shape=[-1, 1]) - torch.reshape(s_pre, shape=[-1, 1])
        diff2I = torch.reshape(i_obs[1: input_size], shape=[-1, 1]) - torch.reshape(i_pre, shape=[-1, 1])
        diff2R = torch.reshape(r_obs[1: input_size], shape=[-1, 1]) - torch.reshape(r_pre, shape=[-1, 1])
        diff2D = torch.reshape(d_obs[1: input_size], shape=[-1, 1]) - torch.reshape(d_pre, shape=[-1, 1])
    ...
    # 修正的 Euler 迭代方法
    def Modify_Euler(self, t=None, input_size=100, s_obs=None, i_obs=None, r_obs=None, d_obs=None, loss_type='l2_loss',
                     scale2lncosh=0.5, scale2beta=1.0, scale2gamma=0.1, scale2mu=0.05):
        assert (t is not None)
        assert (s_obs is not None)
        assert (i_obs is not None)
        ...
        s_bar = torch.reshape(s_bar, shape=[-1, 1])
        i_bar = torch.reshape(i_bar, shape=[-1, 1])
        r_bar = torch.reshape(r_bar, shape=[-1, 1])
        d_bar = torch.reshape(d_bar, shape=[-1, 1])
        ...
        s_pre = 0.5 * (s_bar + torch.reshape(s_tide, shape=[-1, 1]))
        i_pre = 0.5 * (i_bar + torch.reshape(i_tide, shape=[-1, 1]))
        r_pre = 0.5 * (r_bar + torch.reshape(r_tide, shape=[-1, 1]))
        d_pre = 0.5 * (d_bar + torch.reshape(d_tide, shape=[-1, 1]))
    ...
    def evalue_EulerDNN(self, t=None, s_init=10.0, i_init=10.0, r_init=10.0, d_init=10.0, size2predict=7,
                        opt2itera='forward_euler', scale2beta=1.0, scale2gamma=0.1, scale2mu=0.05):
        ...
        if str.lower(opt2itera) == 'forward_euler' or str.lower(opt2itera) == 'quasi_euler':
            for i in range(size2predict):
                ...
                S_list.append(torch.reshape(s_update, shape=[-1, 1]))
                I_list.append(torch.reshape(i_update, shape=[-1, 1]))
                R_list.append(torch.reshape(r_update, shape=[-1, 1]))
                D_list.append(torch.reshape(d_update, shape=[-1, 1]))
            ...
            for i in range(size2predict):
                ...
                i_bar = i_base + nn_beta[i, 0] * s_base * i_base / (s_base + i_base) - \
                        nn_gamma[i, 0] * i_base - nn_mu[i, 0] * i_base
                r_bar = r_base + nn_gamma[i, 0] * i_base
                d_bar = d_base + nn_mu[i, 0] * i_base

                s_tide = s_base - nn_beta[i, 0] * s_bar * i_bar / (s_bar + i_bar)
                i_tide = i_base + nn_beta[i, 0] * s_bar * i_bar / (s_bar + i_bar) - nn_gamma[i, 0] * i_bar - \
                         nn_mu[i, 0] * i_bar
                ...
                r_base = r_update
                d_base = d_update
        S = torch.concat(S_list, dim=0)
        I = torch.concat(I_list, dim=0)
        R = torch.concat(R_list, dim=0)
        D = torch.concat(D_list, dim=0)
    ...
    def evalue_EulerDNN_FixedParas(self, t=None, s_init=10.0, i_init=10.0, r_init=10.0, d_init=10.0, size2predict=7,
                                   opt2itera='forward_euler', opt2fixed_paras='last2train', mean2para=3, scale2beta=1.0,
                                   scale2gamma=0.1, scale2mu=0.05):
        assert (t is not None)
        shape2t = t.shape
        lenght2t_shape = len(shape2t)
        assert (lenght2t_shape == 2)
        ...
        if opt2fixed_paras == 'last2train':
            nn_beta = scale2beta*ParamsNN[0, 0]
            nn_gamma = scale2gamma*ParamsNN[0, 1]
            nn_mu = scale2mu*ParamsNN[0, 2]
        else:  # 训练过程中最后几天的参数的均值作为固定参数，如三天的参数均值作为固定参数
            nn_beta = scale2beta*torch.mean(ParamsNN[:, 0], dim=0)
            nn_gamma = scale2gamma*torch.mean(ParamsNN[:, 1], dim=0)
            nn_mu = scale2mu*torch.mean(ParamsNN[:, 2], dim=0)
...
def solve_SIRD(R):
    ...
    params2Net = SIRDmodel.DNN.parameters()
    ...
    for epoch in range(R['max_epoch'] + 1):
        ...
        if R['opt2Euler'] == 'quasi_euler':
            params, loss2s, loss2i, loss2r, loss2d = SIRDmodel.Quasi_Euler(
                t=t_batch, input_size=batchsize2train, s_obs=s_obs, i_obs=i_obs, r_obs=r_obs,
                d_obs=d_obs, loss_type=R['loss_type'], scale2lncosh=R['lambda2lncosh'],
                scale2beta=R['scale_modify2beta'], scale2gamma=R['scale_modify2gamma'], scale2mu=R['scale_modify2mu'])
        elif R['opt2Euler'] == 'forward_euler':
            params, loss2s, loss2i, loss2r, loss2d = SIRDmodel.Forward_Euler(
                t=t_batch, input_size=batchsize2train, s_obs=s_obs, i_obs=i_obs, r_obs=r_obs,
                d_obs=d_obs, loss_type=R['loss_type'], scale2lncosh=R['lambda2lncosh'],
                scale2beta=R['scale_modify2beta'], scale2gamma=R['scale_modify2gamma'], scale2mu=R['scale_modify2mu'])
        elif R['opt2Euler'] == 'modify_euler':
            params, loss2s, loss2i, loss2r, loss2d = SIRDmodel.Modify_Euler(
                t=t_batch, input_size=batchsize2train, s_obs=s_obs, i_obs=i_obs, r_obs=r_obs,
                d_obs=d_obs, loss_type=R['loss_type'], scale2lncosh=R['lambda2lncosh'],
                scale2beta=R['scale_modify2beta'], scale2gamma=R['scale_modify2gamma'], scale2mu=R['scale_modify2mu'])
        ...
        loss = loss2s + loss2i + loss2r + 10*loss2d + Paras_PWB
        ...
        if epoch % 1000 == 0:
            ...
            paras_nn, S_predict, I_predict, R_predict, D_predict = SIRDmodel.evalue_EulerDNN(
                t=new_test_t_bach, s_init=init2S_test, i_init=init2I_test, r_init=init2R_test, d_init=init2D_test,
                size2predict=batchsize2test, opt2itera=R['opt2Euler'], scale2beta=R['scale_modify2beta'],
            ...
            fix_paras_nn, S_predict2fix, I_predict2fix, R_predict2fix, D_predict2fix = \
                SIRDmodel.evalue_EulerDNN_FixedParas(t=last_train_t, s_init=init2S_test, i_init=init2I_test,
                                                     r_init=init2R_test, d_init=init2D_test,
                                                     size2predict=batchsize2test, opt2itera=R['opt2Euler'],
    ...
    # plotData.plot_train_paras2SIRD(paras_nn[:, 0], para_name='Beta', file_name='Beta2train', coord_points=train_date,
    #                                outPath=R['FolderName'])
    #
    # plotData.plot_train_paras2SIRD(paras_nn[:, 1], para_name='Gamma', file_name='Gamma2train', coord_points=train_date,
    #                                outPath=R['FolderName'])
    #
    # plotData.plot_train_paras2SIRD(paras_nn[:, 2], para_name='Mu', file_name='Mu2train', coord_points=train_date,
    #                                outPath=R['FolderName'])
...
if __name__ == "__main__":
    ...
    R['penalty2weight_biases'] = 0.000001  # Regularization parameter for weights
    ...
    if R['model2NN'] == 'Fourier_DNN':
        R['hidden_layers'] = (75, 100, 80, 80, 50)
    elif R['model2NN'] == 'Fourier_SubDNN':
        R['hidden_layers'] = (6, 10, 5, 5, 4)
    else:
        R['hidden_layers'] = (150, 100, 80, 80, 50)
    ...
    # R['name2act_out'] = 'relu'     # 效果一般
    R['name2act_out'] = 'sigmoid'
    # R['name2act_out'] = 'gauss'
    # R['name2act_out'] = 'square'   # 震荡比较厉害
    ...
    if R['model2NN'] == 'Fourier_DNN':
        ...
        R['freq'] = np.arange(1, 30)  # 网络的频率范围设置
